...
本计划以 `agent-admin/backend` 的 Laravel 9 云控后端为业务中心，新增 `agent-web` 和 `agent-mobile`，并通过共享包避免请求结构和状态机在多端漂移。
...
- Git：`main` 已同步到 `https://github.com/cangerx/zihui`，当前基线提交为 `8f3b739`。
- 已有规划契约：`.agents/runtime/harness/2026-08-26-multichannel-platform/` 下的 `product_spec.json` 和 `round-01-contract.json`。

开发前必须先处理：`agent-admin/frontend/package-lock.json` 与 `package.json` 不同步、Node/PHP 版本偏离项目要求、依赖安全公告、Electron `webSecurity: false` 和全局忽略证书错误等问题。
...
```text
       ...
       agent-desktop          agent-web            agent-mobile
        Electron/Vue          Vue 3 Web          uni-app Vue 3
...
| 层 | 方案 | 约束 |
| --- | --- | --- |
| Web | Vue 3 + TypeScript + Vite + Pinia + Vue Router | 不引用 Electron、Node 原生模块或绝对文件路径 |
| H5/小程序 | uni-app Vue 3 + TypeScript + Pinia + uni-ui | 页面逻辑共享，平台差异放在 adapter，不直接使用浏览器 API |
...
| 包管理 | 新 workspace 采用 pnpm 并锁定版本；现有子项目首轮保持各自 lock | 首轮不做全仓库包管理器迁移，避免无关锁文件变更 |

## 4. 分阶段路线

时间按 1 名后端、2 名前端、1 名 QA/DevOps 的并行小团队估算；单人开发应按人日累加。每阶段完成后再进入下一阶段的硬门禁。
...
### 阶段 1：共享契约与 API v1（5-7 个工作日）

任务：

1. 创建 `packages/contracts`：用户、渠道、错误、分页、模型、权限、对话、消息、任务、资源、余额、套餐和订单类型。
2. 创建 `packages/validators`：请求参数、响应 envelope、任务状态迁移、上传元数据校验。
3. 创建 `packages/api-client`：请求超时、重试、取消、错误映射、Cookie/Bearer 策略、SSE 客户端和平台 adapter 接口。
4. 创建 `packages/domain`：任务轮询、分页、会话列表、余额刷新等跨端纯业务状态。
5. 在 Laravel 中新增 `routes/app.php` 和 `App\Http\Controllers\App\`，挂载 `/api/app/v1`，不删除旧 `routes/api.php`。
6. 统一 `request_id`、幂等键、错误码和审计日志。
...
进入阶段 2 的条件：契约包能被 Web 和 uni-app 引入；Laravel Feature Test 覆盖登录、`me`、模型、对话、图片任务、余额和 IDOR；旧桌面路由回归通过。

### 阶段 2：Web MVP（8-12 个工作日）
...
1. `agent-web` 使用 Vue 3 + Vite + Pinia，所有服务端数据经 `api-client` 获取。
...
进入阶段 3 的条件：浏览器无 `window.api` 时可运行；登录刷新保持、SSE 对话、图片任务全链路和两个用户的 IDOR 测试通过。

### 阶段 3：uni-app H5 与小程序骨架（7-10 个工作日）

任务：

1. 创建 `agent-mobile`，接入共享 contracts、api-client 和 domain。
2. 完成登录、首页、对话、任务、我的页面和统一底部导航。
...
交付物：`npm run build:h5`、小程序 dev 构建、登录和对话占位链路、平台差异清单。

进入阶段 4 的条件：H5 在真实手机浏览器完成登录和对话；小程序在开发者工具完成登录、会话保持、任务查询；依赖图无 Electron、Node 原生模块和 `better-sqlite3`。

### 阶段 4：微信登录、支付与运营能力（8-12 个工作日，依赖资质）
...
进入阶段 5 的条件：沙箱或小额真实支付闭环、重复通知幂等、退款/关闭订单、登录绑定和隐私审查通过。

### 阶段 5：云端图库、知识库和多端同步（10-15 个工作日）
...
进入阶段 6 的条件：资源生命周期和成本监控上线；跨端同步冲突有可重复测试；任务队列重试、死信和人工处理路径明确。

### 阶段 6：灰度发布与正式上线（5-8 个工作日）
...
4. `mobile.yml`：H5 构建、小程序编译检查、共享包依赖扫描。
...
| 风险 | 影响 | 处理与决策点 |
| --- | --- | --- |
| 微信主体、支付和域名资质未齐 | 小程序/支付延期 | 阶段 3 先交付 mock adapter；资质齐全后才排真实联调 |
...
1. 合并本规划和既有 `product_spec.json`、`round-01-contract.json` 到 Git。
2. 修复 admin frontend lock，确定 Node 20/PHP 8.0 CI 镜像。
3. 建立 `packages/` workspace 和 `agent-web`/`agent-mobile` 空工程，不接业务页面。
4. 先实现 contracts、错误模型、认证 client 和 `/api/app/v1/auth/*`、`/bootstrap`。
5. 用 fake provider 打通 Web 登录、对话、图片任务全链路，再接真实供应商。
6. 同步补齐 PHPUnit、Vitest/Playwright 和 IDOR 测试；测试未通过不得开始微信支付开发。
