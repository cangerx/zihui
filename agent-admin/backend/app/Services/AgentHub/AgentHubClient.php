...
{
    private const PATH_PREFIX = '/api/agent-hub';
    ...
    {
        if (!$this->isReady()) {
            throw new RuntimeException('agent_hub_not_configured');
        }
    ...
    private function request(string $method, string $path, array $options = []): Response
    {
        ...
        try {
            ...
            Log::warning('[AgentHubClient] connection failed', [
                'method' => $method,
                'url' => $url,
                'msg' => $e->getMessage(),
            ]);
            throw new RuntimeException('agent_hub_unreachable');
        }
    }
}
