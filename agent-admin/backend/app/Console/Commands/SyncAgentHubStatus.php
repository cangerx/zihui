...
class SyncAgentHubStatus extends Command
{
    protected $signature = 'agent-hub:sync-status
                            {--limit=200 : 单次同步条数上限（1-1000，默认 200）}';

    protected $description = '与智能体共享库同步本地智能体的 hub 状态';

    private const HUB_STATUS_BATCH_SIZE = 100;

    public function handle(AgentHubClient $hub): int
    {
        if (!$hub->isReady()) {
            $this->info('[AgentHub] sync-status: hub not ready, skip.');
            return self::SUCCESS;
        }
        ...
        if ($rows->isEmpty()) {
            $this->info('[AgentHub] sync-status: nothing to sync');
            return self::SUCCESS;
        }

        $this->info("[AgentHub] sync-status: syncing {$rows->count()} rows");
        $totalSynced = 0;
        $totalCleared = 0;
        $totalErrors = 0;
        ...
        foreach ($rows->chunk(self::HUB_STATUS_BATCH_SIZE) as $chunk) {
            ...
            try {
                ...
                Log::warning('[AgentHub] sync-status hub call failed', [
                    'error' => $e->getMessage(),
                    'count' => $chunk->count(),
                ]);
            ...
            if (!$resp->successful()) {
                Log::warning('[AgentHub] sync-status hub returned error', [
                    'status' => $resp->status(),
                    'body' => $resp->json(),
                    'count' => $chunk->count(),
                ]);
            ...
            foreach ($items as $item) {
                ...
                if (!$local) continue;
                Agent::where('id', $local->id)->update([
            ...
            if (!empty($missing)) {
                ...
                if (!empty($missingLocalIds)) {
                    Agent::whereIn('id', $missingLocalIds)->update([
                        'hub_shared_id' => null,
                        'hub_status' => null,
                        'hub_status_synced_at' => $now,
                    ]);
                    $totalCleared += count($missingLocalIds);
                    Log::info('[AgentHub] sync-status detected hub-side removal, cleared local', [
        ...
        $this->info("[AgentHub] sync-status done. synced={$totalSynced} cleared={$totalCleared} errors={$totalErrors}");
        return self::SUCCESS;
    }
}
