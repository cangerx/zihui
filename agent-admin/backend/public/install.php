 ...
 *   - 密码用 password_hash(PASSWORD_BCRYPT)，与 Laravel bcrypt() 兼容
 */
...
function action_check_env(): array
{
    ...
    // 必启函数（不能被 disable_functions 禁用）
    $disabled = array_map('trim', explode(',', (string) ini_get('disable_functions')));
    $needFns  = ['exec', 'popen', 'proc_open', 'putenv', 'symlink'];
    ...
    try {
        $pdo = new PDO(
            ...
            [
                ...
                PDO::ATTR_TIMEOUT    => 5,
                // pdo_mysql 5.x+ 识别 connect_timeout，避免 host 不可达时坚死
        ...
        // 版本兼容性检测（utf8mb4 + 联合索引需要 MySQL 5.7.7+ / 8.0+ / MariaDB 10.2+）
        // MariaDB VERSION() 形如 "5.5.5-10.5.10-MariaDB-..."，前缀 5.5.5 是假版本只为兼容 MySQL 客户端
        ...
        if (!$compatible) {
            $hint = $isMariaDB
                ? "数据库版本 {$version} 不兼容：MariaDB 需要 10.2+ 才能支持 utf8mb4 长索引。"
                  . "请升级 MariaDB 后重新进入安装向导。"
                : "数据库版本 {$version} 不兼容：需要 MySQL 5.7.7+ 或 8.0+。"
                  . "本应用使用 utf8mb4 字符集 + 联合索引，旧版 MySQL（5.6.x 已 EOL / 5.7.6-）"
                  . "InnoDB 索引上限为 767 bytes，会在 migrate `cloud_models` 等表时报"
                  . "「Specified key was too long」错误。请通过宝塔面板「软件商店 → MySQL → 切换版本」"
                  . "一键升级到 5.7+ 或 8.0+（数据自动保留），重启后重新进入本向导。";
            return [
                'ok'    => false,
                'error' => $hint,
        ...
        // 即使版本达标，也额外检查 innodb_large_prefix（MySQL 8.0 已移除此变量）
        // 5.7.7+ 默认 ON，被人手动改 OFF 仍会撞 767 上限，给软警告但不阻塞
        $ilpWarn = '';
...
function action_install(): array
{
    ...
    if ($siteTitle === '')                                    $errors[] = '站点标题必填';
    if (mb_strlen($siteTitle) > 50)                           $errors[] = '站点标题不超过 50 字';
    ...
    if ($adminEmail !== '' && !filter_var($adminEmail, FILTER_VALIDATE_EMAIL)) $errors[] = '管理员邮箱格式无效';
    ...
    // 2. 清 bootstrap/cache 下的静态缓存（config / routes / services / packages）
    //    Laravel 启动时会优先读 cached config.php，不清会覆盖刚写的 .env。
    ...
    // 4. 创建管理员
    //    重装场景：上次安装失败但已 INSERT 过用户，现在换了 username 但 email 一样
    //    会被 unique 约束拦住。同时清同 username 和同 email 的孤儿。
    $now = date('Y-m-d H:i:s');
    try {
        if ($adminEmail !== '') {
            $pdo->prepare('DELETE FROM users WHERE username = ? OR email = ?')
                ->execute([$adminUser, $adminEmail]);
        } else {
            $pdo->prepare('DELETE FROM users WHERE username = ?')
                ->execute([$adminUser]);
        }
        $stmt = $pdo->prepare(
            'INSERT INTO users (username, email, password, role, status, nickname, remark, created_at, updated_at)
             VALUES (?, ?, ?, "admin", "active", ?, "", ?, ?)'
        );
        $stmt->execute([
            $adminUser,
    ...
    // 5. 写入站点标题（system_settings.site_title）
    try {
        $stmt = $pdo->prepare(
            'INSERT INTO system_settings (`key`, `value`, `remark`, `created_at`, `updated_at`)
             VALUES (?, ?, ?, ?, ?)
        ...
        return ['ok' => false, 'error' => '写入站点配置失败：' . $e->getMessage()];
    }
...
# ==== 在线更新源（留空则使用授权端 AGENT_BUILD_BASE_URL/api/updates/admin/version.json）====
UPDATE_CHECK_URL=
UPDATE_RELEASES_URL=
# 后续 zip 放到 COS 时，把 COS 域名加在这里（逗号分隔）
# UPDATE_ALLOWED_ZIP_HOSTS=your-bucket.cos.ap-xxx.myqcloud.com
UPDATE_BACKUP_KEEP_COUNT=5
ENV;
...
{
    // 双引号包裹的字段（APP_NAME / DB_PASSWORD）：转义内部双引号
    if (in_array($key, ['APP_NAME', 'DB_PASSWORD'], true)) {
...
<style>
  * { box-sizing: border-box; }
  ...
  .header h1 { margin: 0 0 16px 0; font-size: 22px; font-weight: 600; }
  .steps {
    display: flex;
    gap: 0;
    font-size: 13px;
  ...
  .summary { background: #fafafa; border: 1px solid #f0f0f0; border-radius: 6px; padding: 12px 16px; font-size: 13px; }
  .summary dt { color: #8c8c8c; display: inline-block; min-width: 90px; }
  .summary dd { display: inline; margin: 0; color: #262626; }
  .summary .row-item { padding: 4px 0; border-bottom: 1px dashed #f0f0f0; }
  .summary .row-item:last-child { border-bottom: 0; }
  ...
  .done-screen { text-align: center; padding: 32px 0; }
  ...
  .done-screen h2 { color: #389e0d; margin-bottom: 8px; }
  .done-screen p { color: #595959; margin: 8px 0; }
</style>
</head>
<body>
<div class="wrap">
  <div class="card">
    <div class="header">
      ...
      <div class="steps">
        ...
        <div class="step" data-step="4">4. 站点信息</div>
      </div>
    </div>
    ...
    <div class="body" id="body">
      ...
      <div class="step-pane" data-step="1">
        <h2>环境检测</h2>
        <p class="hint">检测 PHP 版本、扩展和函数是否满足部署要求。任何一项失败都需要先解决再继续。</p>
      ...
      <div class="step-pane" data-step="2" style="display:none">
        <h2>文件权限检测</h2>
        <p class="hint">检测 storage、bootstrap/cache 等目录是否可写。Linux 下常见解决方案：<code>chmod -R 775 storage bootstrap/cache</code> 并 <code>chown -R www:www</code>。</p>
      ...
      <div class="step-pane" data-step="3" style="display:none">
        <h2>数据库连接</h2>
        <p class="hint">填写 MySQL 连接信息。建议使用 utf8mb4 字符集。点击「测试连接」确认能连通后才能继续。</p>
        <div id="db-alert"></div>
        <div class="row">
          <div class="form-row">
            <label>主机 <span class="req">*</span></label>
            <input type="text" id="db_host" value="127.0.0.1">
            <div class="extra">默认本地地址 127.0.0.1。本机部署时填 127.0.0.1，远程数据库填对应 IP / 域名。</div>
          </div>
        ...
        <div class="form-row">
          ...
          <input type="text" id="db_database" placeholder="例如：agent_admin">
          <div class="extra">需要提前在 MySQL 里创建好数据库。建议字符集 utf8mb4 / 排序规则 utf8mb4_unicode_ci。</div>
        </div>
        <div class="row">
          <div class="form-row">
      ...
      <div class="step-pane" data-step="4" style="display:none">
        <h2>站点信息与管理员账户</h2>
        <p class="hint">填写完后点击「立即安装」开始执行写入 .env、运行数据库迁移、创建管理员等操作。</p>
        <div id="install-alert"></div>
        <div class="form-row">
          <label>站点标题 <span class="req">*</span></label>
          <input type="text" id="site_title" maxlength="50" placeholder="例如：星河智能管理后台">
          <div class="extra">将显示在浏览器标签和左上角侧边栏。最长 50 个字符。</div>
        </div>
        <div class="row">
          <div class="form-row">
          ...
          <div class="form-row">
            ...
            <input type="email" id="admin_email" placeholder="忘记密码时联系用">
          </div>
        </div>
        <div class="row">
          <div class="form-row">
      ...
      <div class="step-pane" data-step="done" style="display:none">
        <div class="done-screen">
          ...
          <h2>安装完成</h2>
          <p id="done-msg">即将跳转到管理后台。如果未自动跳转，请点击下方按钮。</p>
          <p style="margin-top:16px">
            <a id="done-link" href="/admin/" style="display:inline-block; padding:10px 24px; background:#1677ff; color:#fff; border-radius:6px; text-decoration:none; font-size:14px">进入管理后台</a>
          </p>
        </div>
      </div>
    </div>
...
async function doInstall() {
  const siteTitle = $('#site_title').value.trim();
  ...
  const errs = [];
  if (!siteTitle) errs.push('站点标题必填');
  if (siteTitle.length > 50) errs.push('站点标题不超过 50 字');
  ...
  if (adminEmail && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(adminEmail)) errs.push('邮箱格式无效');
  ...
  $('#install-alert').innerHTML = '<div class="alert alert-info"><span class="spinner"></span>正在安装：写入 .env → 数据库迁移 → 创建管理员 → 写入站点配置 → 锁定安装</div>';
  $('#btn-next').disabled = true;
  ...
  if (data.ok) {
    $('#done-msg').textContent = `管理员账号：${adminUser}，3 秒后自动跳转管理后台...`;
    showStep('done');
    setTimeout(() => { window.location.href = data.redirect || '/admin/'; }, 3000);
  } else {
