...
export function SharedAgentSettingsPage() {
  const qc = useQueryClient()
  const { data, isFetching } = useQuery({
    queryKey: ['agentHub', 'settings'],
    queryFn: agentHubSettingsApi.get,
  })

  const [form, setForm] = useState<AgentHubSettings>({
    approve_threshold: 3,
    reject_threshold: 3,
    report_threshold: 5,
    submit_daily_limit: 50,
  })
  ...
  const saveMut = useMutation({
    mutationFn: (payload: Partial<AgentHubSettings>) => agentHubSettingsApi.update(payload),
    onSuccess: () => {
      message.success('已保存')
      qc.invalidateQueries({ queryKey: ['agentHub', 'settings'] })
    },
  })
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card>
        <Space style={{ marginBottom: 12 }}>
          <Title level={5} style={{ margin: 0, marginRight: 16 }}>数字员工 · 规则</Title>
          <Tag color={activeReviewers > 0 ? 'blue' : 'red'}>当前活跃审核员 {activeReviewers} 位</Tag>
        </Space>

        <Paragraph type="secondary" style={{ marginBottom: 16 }}>
          智能体共享库复用云控端共享库审核员身份。审核员投票达 <Text code>approve / reject</Text> 阈值后自动结算；用户举报达 <Text code>report</Text> 阈值后自动隐藏；分享行为受每日上限限制。
        </Paragraph>
