...
export default function UpdatesPage() {
  ...
  async function handleCheck(showMsg = true) {
    ...
    try {
      ...
      setCheckData(data as CheckResp);
  ...
  function startPolling(logId: number) {
    ...
    pollTimer.current = window.setInterval(async () => {
      try {
        ...
        if (data?.log) {
          setActiveLog(data.log as UpdateLog);
  ...
  async function loadHistory() {
    ...
    try {
      ...
      setHistory((data?.data || []) as UpdateLog[]);
  ...
  async function loadDbCheck(showMsg = true) {
    ...
    try {
      ...
      setDbCheck(data as DbCheckData);
      if (showMsg) {
  ...
  async function handleDbRepair() {
    ...
    try {
      ...
      setDbRepairResult(data as DbRepairResult);
      if (data?.success) {
        message.success('修复完成');
      } else {
        message.error('修复未完全成功：' + (data?.error || ''));
      }
      if (data?.after) setDbCheck(data.after as DbCheckData);
    } catch (e: any) {
  ...
  async function loadReleases(showMsg = true) {
    ...
    try {
      ...
      setReleasesData(data as ReleasesResp);
      setReleasesPage(1);
      if (showMsg) {
  ...
  function renderCheckTab() {
    const writable = checkData?.writable || localInfo?.writable;
    const currentVersion = checkData?.current || localInfo?.version || '-';
    const currentReleasedAt = checkData?.current_released_at || localInfo?.released_at || '-';
            ...
            },
            {
              ...
              render: (_: any, r: UpdateLog) => (
                <Button size="small" type="link" onClick={async () => {
                  ...
                  setDetailLog((data?.log as UpdateLog) || null);
                }}>详情</Button>
              ),
            },
          ]}
